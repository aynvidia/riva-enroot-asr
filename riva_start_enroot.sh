#!/bin/bash
# Copyright (c) 2021, NVIDIA CORPORATION.  All rights reserved.
#
# NVIDIA CORPORATION and its licensors retain all intellectual property
# and proprietary rights in and to this software, related documentation
# and any modifications thereto.  Any use, reproduction, disclosure or
# distribution of this software and related documentation without an express
# license agreement from NVIDIA CORPORATION is strictly prohibited.

# ENROOT VERSION - Modified to work with enroot instead of Docker

check_enroot_version() {
    if ! command -v enroot &> /dev/null; then
        echo "Unable to run enroot. Please check that enroot is installed and functioning."
        exit 1
    fi
    echo "Using enroot container runtime..."
}

block_until_server_alive() {
    # determine if TLS/SSL key & cert are provided
    grpc_tls_args=""
    if [ -n "$ssl_server_key" ] && [ -n "$ssl_server_cert" ]; then
        # DO not verify since self-signed/no CA
        grpc_tls_args+="-tls -tls-client-cert /ssl/server.crt -tls-client-key /ssl/server.key -tls-no-verify "
    fi
    for i in {1..30}
    do
    # Use enroot start with the .sqsh file for health check
    enroot start --root $(basename $image_speech_api).sqsh /bin/grpc_health_probe -addr=localhost:$riva_speech_api_port $grpc_tls_args 2> /dev/null
    rc=$?
    if [ $rc -ne 0 ]; then
      echo "Waiting for Riva server to load all models...retrying in 10 seconds"
      sleep 10
    else
      echo "Riva server is ready..."
      return
    fi
    done
    echo "Health ready check failed."
    echo "Check Riva logs by examining the container process"
    exit 1
}

# BEGIN SCRIPT
check_enroot_version

# load config file
script_path="$( cd "$(dirname "$0")" >/dev/null 2>&1 ; pwd -P )"
if [ -z "$1" ]; then
    config_path="${script_path}/config_enroot.sh"
else
    config_path=$(readlink -f $1)
fi

if [[ ! -f $config_path ]]; then
    echo 'Unable to load configuration file. Override path to file with -c argument.'
    exit 1
fi
source $config_path

# determine required LD_PRELOAD & model_repos based on desired services
ld_preload=""
if [ "$service_enabled_asr" = true ] || [ "$service_enabled_nlp" = true ] || [ "$service_enabled_tts" = true ]; then
    model_repos+=" --model-repo=/data/models/"
    # generate ld_preload based on what's deployed in /data/plugins...
    # Skip LD_PRELOAD generation for now to avoid parsing issues
    # ld_preload+=$(enroot start --root --mount $riva_model_loc:/data $(basename $image_speech_api).sqsh /bin/bash -c "find /data/plugins/*.so -type f ! -size 0 2>/dev/null" | grep '\.so$' | sed 's/\r//g' | paste -sd ':' -)
    ld_preload=""
fi

ssl_vol_args=""
ssl_args=""
# determine if TLS/SSL key & cert are provided
if [ -n "$ssl_server_key" ] && [ -n "$ssl_server_cert" ]; then
  ssl_vol_args+="--mount $ssl_server_cert:/ssl/server.crt --mount $ssl_server_key:/ssl/server.key "
  ssl_args+="--use_ssl --ssl_server_cert /ssl/server.crt --ssl_server_key /ssl/server.key "
  if [ -n "$ssl_root_cert" ]; then
      ssl_vol_args+="--mount $ssl_root_cert:/ssl/root_server.crt "
      ssl_args+=" --ssl_root_cert /ssl/root_server.crt "
  fi
fi

# speech server is required
# check if it's already running first...
container_name="$riva_daemon_speech"
if ! pgrep -f "start-riva.*$riva_speech_api_port" > /dev/null; then
    echo "Starting Riva Speech Services. This may take several minutes depending on the number of models deployed."
    
    # Set up port forwarding and GPU access
    if [[ $riva_target_gpu_family == "tegra" ]]; then
        enroot_run_args="--mount /dev/bus/usb:/dev/bus/usb --mount /dev/snd:/dev/snd"
    else
        enroot_run_args=""
    fi
    
    # Start the container directly from the image
    enroot start \
        --root \
        --rw \
        $enroot_run_args \
        --mount $riva_model_loc:/data \
        $ssl_vol_args \
        --env "LD_PRELOAD=$ld_preload" \
        --env "RIVA_API_KEY=$RIVA_API_KEY" \
        --env "RIVA_API_NGC_ORG=$RIVA_API_NGC_ORG" \
        --env "RIVA_EULA=$RIVA_EULA" \
        --env "RIVA_SERVER_HTTP_PORT=$riva_speech_api_http_port" \
        $(basename $image_speech_api).sqsh \
        start-riva --riva-uri=0.0.0.0:$riva_speech_api_port \
        --asr_service=$service_enabled_asr \
        --tts_service=$service_enabled_tts \
        --nlp_service=$service_enabled_nlp \
        $ssl_args &
    
    # Give it a moment to start
    sleep 2
else
    echo "Riva Speech already running. Skipping..."
fi

block_until_server_alive

# Give terminal access for arm64/embedded to try clients, use -s (server only) to disable
if [[ $riva_target_gpu_family == "tegra" ]]; then
    if [[ "$2" != "-s" ]]; then
        echo "Use this container terminal to run applications:"
        enroot start --root $(basename $image_speech_api).sqsh /bin/bash
    fi
fi
